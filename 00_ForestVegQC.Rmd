...
DataDir<-"C:/Users/jschmit/OneDrive - DOI/Data/NCRN_Veg_2025/"

NCRN<-importNCRN(DataDir)

#QCYears<-c(2008, 2012,2016, 2021)
QCYears<-c(2025)
QCPastYears<-c(2020:2021, 2016, 2012,2008)
QCAllYears<-unique(c(QCYears, QCPastYears))
QCLastSample<-2020:2021
...
- Sub-unit codes are not checked and should be reviewed in general
...
- Exotic Status is not exported for Saplings, Seedlings, Shrubs, Shrub Seedlings, and Trees, but it is for Herbs and Vines. We should be consistent.

- Browsing data is inconsistent: Saplings, Shrubs = Yes / No / unk / blank, Seedlings, Herbs = 1 / 0 / blank, Shrub seedlings = Yes / No / unk,  

#### Plots
- Retired date for retired plots not currently recorded and should be considered.

#### Events
- Need to consider how we can verify verify the correct plots were sampled and none were missed.

- Need to handle Invasive, rare and plot maintenance checks before they were part of interface.
...
- Now that the foliage conditions, tree conditions and vines check-boxes are exported, need to verify they are checked for living shrubs, and  have conditions checked from 2015(?) on, but not checked in other cases.

#### Conditions
- In Tree_Conditions export there is now a way to separate saplings and shrubs from trees (compare to stems export which has Habit and Class fields ) - what checks should be added?
