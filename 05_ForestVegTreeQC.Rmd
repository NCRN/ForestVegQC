...
Trees_Standing_Dead_Many_Stems<-QCTrees %>% filter(Status %in% c("Dead Standing","Dead Leaning") & (Stems<1 | Stems >5)) %>% 
  select(Unit_Code, Plot_Name, Sample_Year, Tag, Latin_Name, Status, Stems)
...
#Dead - Human Action has no expectations, so not checked
...
Trees_Dead_Small_BA<-QCTrees %>% 
  filter(Status %in% c("Dead Standing","Dead Leaning"), SumDeadBasalArea_cm2 <round(25*pi,1) ) %>% 
...
#5.24.5
Missed_Trees<-QCLastTrees |> filter(!Status %in% c("Dead Fallen", "Dead - Too Small")) |> 
  select(Tag, Latin_Name, Plot_Name,Status) |> 
  arrange(Plot_Name) |> 
  anti_join(QCTrees,by="Tag") |> anti_join(QCSaplings,by="Tag") |> anti_join(QCTags_Latest |> filter(Missing_Events>1),by="Tag") |> 
  semi_join(QCEvents, by="Plot_Name")

### Multi-Year Checks

Repeat_Trees<-if(length(QCAllYears)!=1) {
  QCAllTrees %>% group_by(Tag) %>% summarize(Records=n()) %>% filter(Records>1) %>% pull(Tag)} else {
    NULL
  }
...
Trees_Live_Large_DBH_change <-if(length(Repeat_Trees>0)){
  QCAllTrees %>% filter(Tag %in% Repeat_Trees) %>% 
    ...
    filter(any(Status %in% Live_Status & lead(Status, order_by = Sample_Year) %in% Live_Status & 
                 abs(Equiv_Live_DBH_cm -lead(Equiv_Live_DBH_cm, order_by=Sample_Year) > 3))) %>%
    mutate(Start_Year=min(Sample_Year)) %>% arrange(Sample_Year) %>% 
    select(Plot_Name, Cycle, Sample_Year, Tag, Latin_Name, LiveDBH=Equiv_Live_DBH_cm, Start_Year, DBH_Check) %>% 
    left_join(QCStemSummary |> select(Tag, Sample_Year, Live_Stems)) |> 
    pivot_wider(id_cols = c(Plot_Name, Tag, Latin_Name, Start_Year), names_prefix = "Cycle ",
                values_from = c(Live_Stems, LiveDBH, DBH_Check), names_from = Cycle, names_sep = " " ) } else data.frame()
...
Trees_Dead_Large_DBH_change <-if(length(Repeat_Trees>0)){
  ...
  mutate(Start_Year=min(Sample_Year)) %>% arrange(Sample_Year) %>% 
   select(Plot_Name, Cycle, Sample_Year, Tag, Latin_Name, DeadDBH=Equiv_Dead_DBH_cm, Start_Year, DBH_Check) %>% 
    left_join(QCStemSummary |> select(Tag, Sample_Year, Dead_Stems)) %>% 
    {if(nrow(.)>0)  pivot_wider(.,id_cols = c(Plot_Name, Tag, Latin_Name, Start_Year), names_prefix = "Cycle ",
               values_from = c(Dead_Stems, DeadDBH, DBH_Check), names_from = Cycle, names_sep = " " ) else data.frame()}  } else data.frame()
...
if(nrow(Trees_Live_Many_Stems)==0) cat("All live tree records have between 1 and 5 stems.<br>") else {
   errorText("Live trees with less than 1 or more than 5 stems:"); NiceTable(Trees_Live_Many_Stems)}
...
```{r Tree_Missed, echo=FALSE, hold=FALSE, results="asis"}
 
  if(nrow(Missed_Trees)==0) cat("All trees that should have been monitored were monitored.<br>") else {
     errorText("Trees that should have been monitored but were not:"); NiceTable(Missed_Trees)}
