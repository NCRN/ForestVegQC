...
Repeat_Quads<-QCQuads %>% group_by(Plot_Name, Quadrat_Number, Sample_Year) %>% summarize(Visits=n()) %>% filter(Visits>1) %>% 
  select(Plot_Name, Quadrat_Number, Sample_Year) %>% ungroup
...
cat("<br>**Quadrats Repeated**<br>") 
  if(nrow(Repeat_Quads)==0) cat("No quadrats were sampled twice in the same year. <br>") else {
     errorText("Quads that were sampled more than once in a single year:"); NiceTable(Repeat_Quads)}
```
