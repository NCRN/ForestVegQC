...
#5.13.0
Trees_No_BA<-QCTrees %>% 
  filter(is.na(SumDeadBasalArea_cm2)|is.na(SumDeadBasalArea_cm2)| SumLiveBasalArea_cm2<0 | SumDeadBasalArea_cm2<0) %>% 
...
#5.21.0
Trees_Live_Redundant_Conditions<-QCConditions %>% filter( (Sample_Year %in% QCYears) & (Status %in% Live_Status)) %>% 
  group_by(Tag, Sample_Year) %>% filter(n()!=n_distinct(Condition)) %>% 
  ...
  mutate(Start_Year=min(Sample_Year)) %>% arrange(Sample_Year) %>% 
   select(Plot_Name, Cycle, Sample_Year, Tag, Latin_Name, DeadDBH=Equiv_Dead_DBH_cm, Stems, Start_Year) %>% 
    {if(nrow(.)>0)  pivot_wider(.,id_cols = c(Plot_Name, Tag, Latin_Name, Start_Year), names_prefix = "Cycle ",
               values_from = c(Stems, DeadDBH), names_from = Cycle, names_sep = " " ) else data.frame()}  } else data.frame()
...
# 5.43.0
 Trees_Start_Too_Large <-if(length(Repeat_Trees>0)){
  ...
  filter(!Tag %in% (semi_join(QCTrees, QCEvents %>% group_by(Plot_Name) %>% filter(rank(Event_Year)==1)) %>% pull (Tag)),
     Tag %in% Repeat_Trees) %>% 
  group_by(Tag) %>% arrange(Tag, Sample_Year) %>% 
...
if(nrow(Trees_Bad_Unit)==0) cat("All trees have a recognized Unit Code.<br>") else {
   cat("<span style='color:red;'> Trees with a missing or unrecongized Unit Code.</span><br>"); NiceTable(Trees_Bad_Unit)}

if(nrow(Trees_Bad_Unit_Group)==0) cat("All trees have a recognized Unit Group.<br>") else {
   cat("<span style='color:red;'> Trees with a missing or unrecongized Unit Group.</span><br>"); NiceTable(Trees_Bad_Unit_Group)}
 
 if(nrow(Trees_Mismatch_Unit_Group)==0) cat("All trees have Unit Group that is consistent with their Unit Code.<br>") else {
    cat("<span style='color:red;'> Trees with a Unit Group that is inconsistent with their Unit Code.</span><br>");NiceTable(Trees_Mismatch_Unit_Group)}
...
if(nrow(Trees_BA_Stems_Mistmatch)==0) cat("No trees have a stems with no BA or BA with no stems.<br>") else {
...
cat("<br>**Redundant Tree Conditions**<br>")
if(nrow(Trees_Live_Redundant_Conditions)==0) cat("All tree conditions are only entered only once.<br>") else {
  errorText("Living tree records with a repeated conditions:"); NiceTable(Trees_Live_Redundant_Conditions)}
