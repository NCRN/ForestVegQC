...
#2.2.0
Events_No_Protocol<-QCEvents %>% filter(Protocol_Name=="" | is.na(Protocol_Name)) %>% select(Unit_Code, Plot_Name, Event_Year, Protocol_Name)

#2.3.0
Events_Bad_Protocol<-QCEvents %>% filter(Protocol_Name!=""| is.na(Protocol_Name)) %>%
  filter( (Event_Year<2009 & Protocol_Name != "1.0") | (Event_Year==2009 & Protocol_Name != "2.0") |
    ...
    (Event_Year==2018 & Protocol_Name != "2.4") | (Event_Year==2019 & Protocol_Name!= "2.5") |
     (Event_Year==2020 & Protocol_Name!= "2.6") ) %>% 
