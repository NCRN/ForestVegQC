...
if(nrow(Saplings_Bad_Status)==0) cat(paste(TotSaplings, " Sapling records checked and all have a valid status.  \n")) else {
   cat("Sapling records with a missing status or a status we do not use:  \n"); NiceTable(Saplings_Bad_Status)}
...
```{r Sapling_Species, echo=FALSE, hold=FALSE, results="asis"}
  ...
  if(nrow(Saplings_No_Name)==0) cat(paste(TotSaplings, " Sapling records checked and all have a species ID.  \n")) else {
     cat("Saplings records with no species name:  \n"); NiceTable(Saplings_No_Name)}
...
```{r Sapling_Tag, echo=FALSE, hold=FALSE, results="asis"}
  ...
  if(nrow(Saplings_Bad_Tag)==0) cat(paste(TotSaplings, " Sapling records checked and all have a valid tag.  \n")) else {
     cat("Sapling records with no tag or a tag <1 or >30,000 tag:  \n"); NiceTable(Saplings_Bad_Tag)}
...
```{r Sapling_Diameter, echo=FALSE, hold=FALSE, results="asis"}
  ...
  if(nrow(Saplings_Bad_DBH_Check)==0) cat(paste(TotSaplings, " Sapling records checked and none have an inappropriate DBH check.  \n")) else { 
     cat("Sapling records with the DBH check box checked before that was done or for tree with no DBH:  \n"); NiceTable(Saplings_Bad_DBH_Check)}
 
  if(nrow(Saplings_Live_Bad_DBH)==0) cat(paste(TotSaplings, " Saplings records checked and no live trees have an invlid DBH.  \n")) else { 
     cat("Live sapling records with a living DBH=0 or > 10cm:  \n"); NiceTable(Saplings_Live_Bad_DBH)}
  
 if(nrow(Saplings_Dead_Bad_DBH)==0) cat(paste(TotSaplings, " Sapling records checked and no dead trees have a mising or live DBH.  \n")) else { 
     cat("Dead sapling records with a living or missing DBH:  \n"); NiceTable(Saplings_Dead_Bad_DBH)}
 
 if(nrow(Saplings_Other_Bad_DBH)==0) cat(paste(TotSaplings, " Sapling records checked and no saplings have a live DBH when they should not.  \n")) else     { cat("Sapling records with a live DBH that should not have one:  \n"); NiceTable(Saplings_Other_Bad_DBH)}
...
```{r Sapling_Stem_Diameter, echo=FALSE, hold=FALSE, results="asis"}
  ...
  if(nrow(Sapling_Stems_Live_Bad_DBH)==0) cat(paste(TotStems, " Stem records checked and no live sapling stems have an invlid DBH.  \n")) else { 
     cat("Live sapling stem records with a missing DBH,  DBH <1 cm or DBH >0 10cm:  \n"); NiceTable(Sapling_Stems_Live_Bad_DBH)}
  
  if(nrow(Sapling_Stems_Dead_Bad_DBH)==0) cat(paste(TotStems, " Stem records checked and no dead sapling stems have an invlid DBH.  \n")) else { 
     cat("Dead sapling stem records with a missing DBH or a DBH <10 cm:  \n"); NiceTable(Sapling_Stems_Dead_Bad_DBH)}
  
  if(nrow(Sapling_Stems_Other_Bad_DBH)==0) cat(paste(TotStems, " Stem records checked and no sapling stems have a DBH when they should not.  \n")) else { 
     cat("Sapling stem records with a DBH on a tree that should not have any DBH:  \n"); NiceTable(Sapling_Stems_Other_Bad_DBH)}
...
```{r Sapling_vigor, echo=FALSE, hold=FALSE, results="asis"}
  ...
  if(nrow(Saplings_Live_Bad_Vigor)==0) cat(paste(TotSaplings, " Sapling records checked and all live saplings have a valid vigor.  \n")) else {
     cat("Living sa[;omg] records with a vigor other than 0-5:  \n"); NiceTable(Saplings_Live_Bad_Vigor)}
  
  if(nrow(Saplings_Unalive_Bad_Vigor)==0) cat(paste(TotSaplings, " Sapling records checked and no non-living trees have a vigor.  \n")) else {
     cat("Non-living tree records with a vigor:  \n"); NiceTable(Saplings_Unalive_Bad_Vigor)}
