...
```{r Tree_Diameter, echo=FALSE, hold=FALSE, results="asis"}
 ...
 if(nrow(Trees_Live_Bad_DBH)==0) cat(paste(TotTrees, " Tree records checked and no live trees have an invlid DBH.  \n")) else { 
    cat("Live tree records with a living or dead DBH < 10cm:  \n"); NiceTable(Trees_Live_Bad_DBH)}
...
```{r Tree_Stem_Diameter, echo=FALSE, hold=FALSE, results="asis"}
 ...
 if(nrow(Tree_Stems_Live_Bad_DBH)==0) cat(paste(TotStems, " Stem records checked and no live tree stems have an invlid DBH.  \n")) else { 
    cat("Live tree stem records with a missing DBH or a DBH <1 cm:  \n"); NiceTable(Tree_Stems_Live_Bad_DBH)}

 if(nrow(Tree_Stems_Dead_Bad_DBH)==0) cat(paste(TotStems, " Stem records checked and no dead tree stems have an invlid DBH.  \n")) else { 
    cat("Dead tree stem records with a missing DBH or a DBH <10 cm:  \n"); NiceTable(Tree_Stems_Dead_Bad_DBH)}
 
 if(nrow(Tree_Stems_Other_Bad_DBH)==0) cat(paste(TotStems, " Stem records checked and no tree stems have a DBH when they should not.  \n")) else { 
    cat("Tree stem records with a DBH on a tree that should not have any DBH:  \n"); NiceTable(Tree_Stems_Other_Bad_DBH)}
