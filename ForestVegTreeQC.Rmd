...
Trees_Dead_Large_DBH_change <-if(length(Repeat_Trees>0)){
 ...
 Trees_Lose_EAB<- if(length(Repeat_Trees>0)){
  ...
  group_by(Tag) %>% arrange(Tag, Sample_Year) %>% 
  filter(any( Condition=="Emerald ash borer" & all(lead(Condition != "Emerald ash borer")) )) %>% 
  {if(nrow(.)>0) mutate(., Start_Year=min(Sample_Year)) %>% arrange(Sample_Year) %>%
    filter(Condition=="Emerald ash borer") %>% 
  select(Plot_Name, Cycle, Sample_Year, Tag, Latin_Name, Condition) %>% 
  pivot_wider(id_cols = c(Plot_Name, Tag, Latin_Name, Start_Year), names_prefix = "Cycle ",
...
cat("<br>**Trees that have EAB and then later do not**<br>")
