...
### The following data is not currenty QCed;

- All: Flags are not exported from the database and are not considedered during QC.
...
TotPlots<-nrow(QCPlots)

Plots_No_Unit<-QCPlots %>% filter(is.na(Unit_Code)) %>% select(Unit_Code, Plot_Name)
Plots_Bad_Slope<-QCPlots %>% filter(Slope<0 | Slope >58 | is.na(Slope)) %>% select(Unit_Code, Plot_Name, Slope)
...
TotEvents<-nrow(QCEvents)

Events_No_Protocol<-QCEvents %>% filter(is.na(Protocol_Name)) %>% select(Unit_Code, Plot_Name, Event_Year, Protocol_Name)
#Plots_Bad_Slope<-QCPlots %>% filter(Slope<0 | Slope >58 | is.na(Slope)) %>% select(Unit_Code, Plot_Name, Slope)
#Plots_Bad_Aspect<-QCPlots %>% filter(Aspect<=0 | Aspect >360 | (is.na(Aspect) & !Slope %in% 0)) %>% select(Unit_Code, Plot_Name, Slope, Aspect)
#Plots_No_Coords<-QCPlots %>% filter(is.na(Latitude)| is.na(Longitude) | is.na(UTM_18N_NAD83_X) | is.na(UTM_18N_NAD83_Y)) %>% 
#   select(Unit_Code, Plot_Name, Latitude, Longitude, UTM_18N_NAD83_X, UTM_18N_NAD83_Y)
...
- Protocol_Name - what version of the protocol?
- Event date the correct year (and maybe check for months flag if before April or after October?)
- Pictures taken should be checked
- CWD for all transects checked
- deer impact value present after a certain year
...
## Plot Floor
- All Plot should have a value for 
   - Rock Cover
   - Bare Soild
   - Trampled 
   - what is it with humus and earthworms - old thing?
   - consider what counts as consistent for rock cover

## CWD 
- All should have:
 - Transect
 - Decay Class (1-5)
 - Species that corresponds to a tree species
 - Diameter >= 7.5 and < some big number
 - Hollow as T/F
 - If it has a tag, should match a tree (past or present)
 
## Trees
- Should have a status (alive standing etc)
- should have an Species ID that we recognize as being a tree
- Should have a tag that makes sense -to the extent this is possible. Should not be retired in office generally
- should not start as missing, earlier ones should not start as dead, never start as dead fallen
- should have a crown class in the accepted range, unless dead or missing, and then it should not have a crown class.
- alive trees should have vines, conditions, and foliage checked. other status should not be checked (not sure how EAB works, maybe conditions checked then)
- DBH check should not be checked for missing or dead fallen trees. And only be checked in later data
- Tree vigor should exist only for live trees in later data
- if tree is listed as live it has to have Equiv diameter > 10cm and all measured stems > 1 cm. Can have dead stems > 10cm too
